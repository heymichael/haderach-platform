...
1. [Med] Evolve deploy trigger to PR-promote or event-driven
2. [Med] Extend automatic testing coverage to all apps
3. [Med] Add PR process/template and requirements process/template
4. [Med] Define full local parity contract for multi-app platform testing
5. [Low] Migrate private docs delivery to Cloud Run + IAP (replace temporary static hosting)
6. [Low] Improve deploy-time E2E test visibility in GitHub Actions
7. [Low] Enforce Required PR Gate via branch protection on main
8. [Low] Amend Root routing section after homepage app onboarding
...
**1. Evolve deploy trigger to PR-promote or event-driven**

Purpose: Replace the manual-dispatch deploy workflow with a more auditable and automated trigger so deployments have a clear review trail and/or happen automatically on artifact publication.

Approach: Evaluate two models: (A) PR-promote, where a version manifest file is checked into the platform repo and updating it via PR triggers deploy on merge; (B) event-driven, where the card repo sends a repository_dispatch event to the platform repo after artifact publish, triggering deploy automatically. PR-promote gives audit trail and rollback simplicity. Event-driven reduces manual steps. Implement the chosen model and update `docs/architecture.md` accordingly.

**2. Extend automatic testing coverage to all apps**
...
**3. Add PR process/template and requirements process/template**
...
**4. Define full local parity contract for multi-app platform testing**
...
**5. Migrate private docs delivery to Cloud Run + IAP (replace temporary static hosting)**
...
**6. Improve deploy-time E2E test visibility in GitHub Actions**
...
**7. Enforce Required PR Gate via branch protection on main**
...
**8. Amend Root routing section after homepage app onboarding**
