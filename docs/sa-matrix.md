...
| digital-media-ui-publisher | digital-media-ui-publisher@haderach-ai.iam.gserviceaccount.com | CI/CD artifact publish for heymichael/digital-media-ui repo | Bucket: haderach-app-artifacts | `roles/storage.objectAdmin` | WIF (keyless via GitHub Actions) | N/A — keyless | Created task #300, approved 2026-04-29. ⚠️ Same bucket-wide objectAdmin issue. |
...
| digital-media-publisher | digital-media-publisher@haderach-ai.iam.gserviceaccount.com | CI/CD — build/push digital-media-api image, run migrations, deploy to Cloud Run | AR: haderach-apps repo; Project: haderach-ai; Secret: DIGITAL_MEDIA_DATABASE_URL | `roles/artifactregistry.writer` (AR-scoped), `roles/run.developer` (project), `roles/iam.serviceAccountUser` on digital-media-api-runner, `roles/cloudsql.client` (project), `roles/secretmanager.secretAccessor` on DIGITAL_MEDIA_DATABASE_URL | WIF (keyless via GitHub Actions) | N/A — keyless | Created task #300, approved 2026-04-29. Acts-as `digital-media-api-runner` specifically. `cloudsql.client` and DIGITAL_MEDIA_DATABASE_URL access for SOC 2 compliant CI migrations via Cloud SQL Proxy. |
...
| digital-media-api-runner | digital-media-api-runner@haderach-ai.iam.gserviceaccount.com | Cloud Run runtime identity for digital-media-api | Instance: haderach-digital-media Cloud SQL; Bucket: haderach-media-haderach | `roles/cloudsql.client` (project), `roles/aiplatform.user` (project — Vertex AI embeddings + Vision API), `roles/storage.objectAdmin` (bucket: haderach-media-haderach), `roles/secretmanager.secretAccessor` on DIGITAL_MEDIA_DATABASE_URL | WIF (keyless via Cloud Run) | N/A — keyless | Created task #300, approved 2026-04-29. `cloudsql.client` is project-level; `aiplatform.user` for text-embedding-005 (semantic search) and Vision API (auto-tagging). GCS bucket access is scoped to the media bucket only. |
